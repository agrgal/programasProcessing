class Fruta {
  ...
  Fruta (float miAncho,float screenSize, color miColor) {
    ...
    int numx = int(random(2,numCuadrados-1)); // cuento aleatoriamente los cuadrados de ancho
    int numy = int(random(2,numCuadrados-1)); // idem los altos
    float px = miAncho/2+(numx-1)*miAncho; float py = miAncho/2+(numy-1)*miAncho; // transformo a coordenadas x,y
  ...
  void mostrar() {
      miFruta.mostrar(); // muestro mi cyuadrado.
  }
