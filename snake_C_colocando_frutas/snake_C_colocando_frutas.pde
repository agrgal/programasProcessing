...
int longitudSnake = 11;
float contar = 0;

// Animando nuestra serpiente
float vel = 0.5; // velocidad del juego
...
void setup() {
 ...
 surface.setSize(sizeVentana,int(sizeVentana*1.4)); // redefinición del alto y ancho de la ventana. Margen para poner información
...
sss.mostrar(); 
 ...
 /// D) Muestro información
 textSize(20);
 textAlign(LEFT,CENTER);
 text("Cuadrante: "+str(sss.cuadrante),20,sizeVentana+40);
 text("XX: "+str(sss.miSnake.get(0).x)+" // YY: "+str(sss.miSnake.get(0).y),20,sizeVentana+60);
 text("Hemisferio Norte-Sur: "+str(sss.hemisferioNS()),20,sizeVentana+80);
 text("Hemisferio Este-Oeste: "+str(sss.hemisferioEO()),20,sizeVentana+100);
 text("Colison zona Snake: "+str(sss.colisionZonaSnake(0,0)),20,sizeVentana+120);
 
 // ============================================
 // Refresco del juego
 // ============================================
 // FIN A) Cambio la velocidad del frame
 frameRate(vel*8); // vel=1, un frame por segundo. Sumo 3, porque si no es muy lento.
}
